...
def build_sensor_meta(name: str) -> Dict:
    ...
    # generic fallback (e.g., co2, soil ...)
    upper = n.upper()
    prof = getattr(config, f'{upper}_PROFILE', {"data_type": "float", "min": 0, "max": 100})
    csvp = getattr(config, f'{upper}_CSV', None)
    return {
        "ae": f"C{n}Sensor",
        "cnt": n,                 
        "profile": prof,
...
class MqttOneM2MClient:
    ...
    def _send_request(self, body, ok_rsc=(2000, 2001, 2004)) -> bool:
...
class SensorWorker:
    ...
    def run(self):
        print(f"[{self.sensor_name.upper()}] run (protocol={self.protocol}, mode={self.mode}, period={self.period_ns/1e9}s)")
        ...
        try:
            while not self.stop_flag.is_set():
                ...
                ok = (send_cin_http(self.meta["ae"], self.meta["cnt"], value)
                      if self.protocol == 'http'
                      else self.mqtt.send_cin(self.meta["ae"], self.meta["cnt"], value))

                if ok:
                    print(f"[{self.sensor_name.upper()}] Sent: {value}")
                    self.err = 0
                else:
                    self.err += 1
